...
#pragma warning disable IDE0044, IDE0051, IDE0052, IDE0060, 169, 414, 649
	...
	public sealed class Firestore {
		...
		internal Firestore(AuthObject authObject) {
		...
		public CollectionReference GetCollectionReference(string path = null) => collectionRegex.IsMatch(path)
				? new CollectionReference(authObject, path)
		...
		public DocumentReference GetDocumentReference(string path = null) => documentRegex.IsMatch(path)
				? new DocumentReference(authObject, path)
		...
		private static string CreateQueryString(params (string key, object value)[] queries) {
			NameValueCollection query = new NameValueCollection(capacity: queries.Length);
			foreach ((string key, object value) in queries) {
		...
		private static string AddLeadingSlash(string path = null) => path?.Insert(0, "/") ?? "";
		...
		public sealed class CollectionReference {
			...
			internal CollectionReference(AuthObject authObject, string path = null) => (this.authObject, Path) = (authObject, path);
			...
			public async Task<Document> CreateDocumentAsync(string documentName, Document document) {
				client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", authObject.User?.idToken);
				HttpResponseMessage response = await client.PostAsync($"v1/projects/{authObject.ProjectId}/databases/(default)/documents{AddLeadingSlash(Path)}/{(documentName ?? null)?.Insert(0, "?documentId=")}", new StringContent(document.ToString()));
			...
			public DocumentReference GetDocumentReference(string path) => documentRegex.IsMatch($"{Path}/{path}")
					? new DocumentReference(authObject, $"{Path}/{path}")
					: throw new ArgumentException("Invalid path to document", nameof(path));
			...
			public DocumentReference GetParent() => new DocumentReference(authObject, GetRelativePath(Path));
			...
			public async Task<DocumentList> ListDocumentsAsync(string collectionId = null, int? pageSize = null, string pageToken = null, string orderBy = null) {
				...
				client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", authObject.User?.idToken);
				HttpResponseMessage response = await client.GetAsync($"v1/projects/{authObject.ProjectId}/databases/(default)/documents{AddLeadingSlash(Path)}/{collectionId}{query}");
		...
		public sealed class DocumentReference {
			...
			internal DocumentReference(AuthObject authObject, string path = null) => (this.authObject, Path) = (authObject, path);
			...
			public CollectionReference GetCollectionReference(string path) => collectionRegex.IsMatch($"{Path}/{path}")
					? new CollectionReference(authObject, path: $"{Path}/{path}")
					: throw new ArgumentException("Invalid path to collection", nameof(path));
			...
			public TransactionBuilder TransactionBuilder(TransactionBuilder.TransactionOptions.Mode mode, object param) => new TransactionBuilder(authObject, Path, mode, param);
			...
			public async Task<bool> DeleteAsync() {
				client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", authObject.User?.idToken);
				HttpResponseMessage response = await client.DeleteAsync($"v1/projects/{authObject.ProjectId}/databases/(default)/documents{AddLeadingSlash(Path)}");
			...
			public async Task<Document> GetAsync() {
				client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", authObject.User?.idToken);
				HttpResponseMessage response = await client.GetAsync($"v1/projects/{authObject.ProjectId}/databases/(default)/documents{AddLeadingSlash(Path)}");
			...
			public CollectionReference GetParent() => new CollectionReference(authObject, GetRelativePath(Path));
			...
			public async Task<Document> UpdateAsync(Document document, Precondition currentDocument = null) {
				...
				client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", authObject.User?.idToken);
				HttpResponseMessage response = await client.PatchAsync($"v1/projects/{authObject.ProjectId}/databases/(default)/documents{AddLeadingSlash(Path)}{query}", new StringContent(document.ToString()));
			...
			public async Task<CollectionIds> ListCollectionsAsync(int? pageSize = null, string pageToken = null) {
				client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", authObject.User?.idToken);
				HttpResponseMessage response = await client.PostAsync($"v1beta1/projects/{authObject.ProjectId}/databases/(default)/documents{AddLeadingSlash(Path)}:listCollectionIds", new StringContent(new CollectionListIdContent(pageSize, pageToken).ToString()));
			...
			public sealed class Value {
				...
				public Value(Type type, object value) {
					...
					switch (type) {
						...
						case Type.Integer: {
							IntegerValue = int.TryParse(value.ToString(), out int ignoredInt)
								? value.ToString()
						...
						case Type.Double: {
							DoubleValue = double.TryParse(value.ToString(), out double d)
								? d
						...
						case Type.Timestamp: {
							TimestampValue = value is DateTime date
						...
						case Type.GeoPoint: {
							GeoPointValue = value is LatLon latlon
								? latlon
						...
						case Type.Array: {
							ArrayValue = value is List<Value> array
								? new ArrayValueType(array)
								: throw new ArgumentException("Entered non-array value for an array type value", nameof(value));
							break;
						}

						case Type.Map: {
							MapValue = value is Dictionary<string, Value> map
								? new MapValueType(map)
		...
		public sealed class Transaction {
			...
			internal Transaction(AuthObject authObject, string basePath, string transaction) => (this.authObject, BasePath, this.transaction) = (authObject, basePath, transaction);
			...
			public async Task<bool> Commit() {
				client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", authObject.User.idToken);
				HttpResponseMessage response = await client.PostAsync($"v1/projects/{authObject.ProjectId}/databases/(default)/documents:commit", new StringContent(ToString()));
		...
		public sealed class TransactionBuilder {
			...
			internal TransactionBuilder(AuthObject authObject, string basePath, TransactionOptions.Mode mode, object param) => (this.authObject, BasePath, options) = (authObject, basePath, new TransactionOptions(mode, param));
			...
			public async Task<Transaction> StartTransaction() {
				HttpResponseMessage response = await client.PostAsync($"v1/projects/{authObject.ProjectId}/databases/(default)/documents:beginTransaction", new StringContent(options.ToString()));
				Console.WriteLine(response);
				return response.IsSuccessStatusCode
					? new Transaction(authObject, BasePath, JsonConvert.DeserializeAnonymousType(await response.Content.ReadAsStringAsync(), new { transaction = "" }).transaction)
